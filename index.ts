...
const PRIVATE_KEY_1:string="4830EC8DBE1E91557F6AE6A2EFED706821EEA5AD39FEC431B8C6860F486EF572";
const PUBLIC_KEY_1:string="04779D7BD7B5A316DE28C30FB5B12E215718AA423F42B6C1CDA8EF61E845F2978A58529458D6D4B31FB2328DB62CBB341FC0815172F79C3423C42B7D5A5F1888E2";
const ADDR_1:string="TYJWHBcoRe3cDp79M2R6WiY37EiHn9w7oe";
const ADDR_HEX_1:string="41f4f917ee28322a788a8e712fa6bbcf4f8e149f34";

const PRIVATE_KEY_2:string="9D4ED983D5A0B97CC428DB653B83C391613DF168E6267C39D74426EB19C6A55A";
const PUBLIC_KEY_2:string="04240FC04FD997E7A466BD592BA364691EBA0AF23AD53BF343687D05667E4CAC1E0E0649112F770E9C5D10350644C4D4FF8B3AE3F0D3993A03AA3131C5E631CCF7";
const ADDR_2:string="THpuXbUBxFWhzUKdTves8HKUJ4CEGoYscG";
const ADDR_HEX_2:string="41562f17a8d2f0897ff5396d646df7bd705ddaa747";
...
async function createConn():Promise<any>{
...
async function getBalance() {
    let addr= ADDR_1;
    let connObj = await createConn();
    let account = await connObj.trx.getAccount(addr);
    console.info("trx balance:", account);
    console.info("TRC10 token:",account.assetV2);

    // 获取带宽与能量相关信息
    let resource = await connObj.trx.getAccountResources(addr)
    console.info("accountResource:", resource);
}

async function getTRC10TokenInfo() {
    let tokenId="1000016";
    
    let connObj = await createConn();
    let tokenInfo = await connObj.trx.getTokenByID(tokenId);
...
async function transferTrx() {
    ...
    // 签名
    let signedTxn = await connObj.trx.sign(tradeObj, PRIVATE_KEY_1);

    // 广播
    const receipt = await connObj.trx.sendRawTransaction(signedTxn);
    console.info("receipt:", receipt);
}

async function transferTrx2() {
    let connObj = await createConn();
...
async function transferTRC10Token() {
    ...
    //创建一个未签名的 TRC10 转账交易
    let connObj = await createConn();
    let tradeobj = await connObj.transactionBuilder.sendToken(
        toAddress,
        amount,
        tokenID,
        fromAddress,    
    ...
    //签名
    const signedtxn = await connObj.trx.sign(
        tradeobj,
        PRIVATE_KEY_1
    );

    //广播
    const receipt = await connObj.trx.sendRawTransaction(
        signedtxn
    );
    console.info("receipt:", receipt);
}

async function getTRC20Info() {
    let connObj = await createConn();

    let contractAddr="TXLAQ63Xg1NAzckPwKHvzw7CSEmLMEqcdj";
    let contractHandler = await connObj.contract().at(contractAddr);
    connObj.setAddress(contractAddr); // 必须得要这句才能正常执行

    let contractName = await contractHandler.name().call();
    console.info("contractName:", contractName);
    let symbol = await contractHandler.symbol().call();
    console.info("symbol:", symbol);
    let decimal = await contractHandler.decimals().call();
    console.info("decimal:", decimal);
    let totalSupply = await contractHandler.totalSupply().call(); // 总供应量
...
async function getTRC20Balance() {
    ...
    let contractObj = await connObj.contract().at(contractAddr);
    connObj.setAddress(contractAddr); // 必须得要这句才能正常执行

    let result = await contractObj.balanceOf(addr).call();
...
async function transferTRC20() {
    ...
    let result = await connObj.transactionBuilder.triggerSmartContract(
        contractAddr, 'transfer(address,uint256)', {
            feeLimit: 10000000,
            callValue: 0
        },
        [{
    ...
    const signature = await connObj.trx.sign(result.transaction, fromPrivateKey);
    console.info("Signature:", signature);
    
    //result.transaction["signature"] = [signature];

    const broadcast = await connObj.trx.sendRawTransaction(signature);
...
async function getTransactionHistory() {
    ...
    let addr= ADDR_1;
    let limit=30;
    let result =await axios.get(`${NET_CONFIG.solidityNode}/v1/accounts/${addr}/transactions?limit=${limit}'`);
...
async function getTransactionItem() {
    let connObj = await createConn();
    let transactionInfo = await connObj.trx.getTransactionInfo("469de0b594442e90c156de748ea899414b4a6a9db4d258b66666e5d9d393f948");
    let jsonStr = JSON.stringify(transactionInfo,null,2);
    console.log("result:", jsonStr);
}

async function decodeTRC20ParamTest() {    
...
async function getTRC10AssestList() {
    let assetResult = await axios.get(`${NET_CONFIG.fullNode}/wallet/getassetissuelist`)
    console.info( assetResult.data);
}

function getTokenType(tokenId:string):number{
